...
tmp <- RFDU2011iris %>%
    ...
    gather(categorie, valeur, RFUCMO11, RFUCQ211, RFUCIQ11, RFUCRD11, RFUCET11, RFUCGI11) %>%
    group_by(categorie) %>%
    summarise_each(funs(moyenne = mean(., na.rm = TRUE),
                        `écart-type` = sd(., na.rm = TRUE)), valeur)
tmp$categorie <- c("Revenu médian", "Revenu moyen", "Écart inter-quartile", "Rapport interdécile", "Écart-type", "Indice de Gini")
...
RFDU2011iris %>%
  filter(str_sub(CodeIris, 1, 5) %in% AMM) %>%
  transmute(`Revenu médian` = RFUCQ211, `Revenu moyen` = RFUCMO11, `Écart inter-quartile` = RFUCIQ11, `Rapport interdécile` = RFUCRD11, `Écart-type` = RFUCET11, `Indice de Gini` = RFUCGI11) %>%
  cor(use = "pairwise") %>%
